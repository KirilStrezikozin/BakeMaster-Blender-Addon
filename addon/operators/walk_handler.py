...
class BM_OT_Global_WalkHandler(Operator):
    ...
    def evaluate_data_trans(self, bakemaster, data, _, attr):
        """
        ...
        bakemaster.allow_drag_trans = bakemaster.wh_has_ms(
            data, attr)
    ...
    def drop_init(self, bakemaster):
        ...
        for data_name in datas:
            ...
            # remove drag_empties
            bakemaster.wh_disable_drag(data, containers, attr)
    ...
    def drag(self, bakemaster):
        ...
        if data is None:
            bakemaster.log("o0x0000", bakemaster.walk_data_name, self)
            self.drag_end(bakemaster)
            return
...
class BM_OT_Global_WalkData_AddDropped(Operator):
    ...
    def remove(self, bakemaster: not None):
        bakemaster.wh_remove(self.data, self.walk_data, self.index)

    def add(self, _: not None, add_names: list):
        if self.walk_data == "bakejobs":
            add_ot = getattr(
                bpy_ops.bakemaster, "%s_add" % self.walk_data[:-1])
            add_ot('INVOKE_DEFAULT')

            self.walk_data = "containers"

            # new_bakejob = bakemaster.get_bakejob(
            #     bakemaster, bakemaster.bakejobs_len - 1)

        for name in add_names:
            add_ot = getattr(
                bpy_ops.bakemaster, "%s_add" % self.walk_data[:-1])
            add_ot('INVOKE_DEFAULT', new_name=name)
    ...
    def invoke(self, context, _):
        ...
        container_getter = getattr(bakemaster, "get_%s" % self.walk_data[:-1])
        self.container = container_getter(self.data, self.index)
...
class BM_OT_Global_WalkData_Move_Lowpoly_Data(Operator):
    """
...
class BM_OT_Global_WalkData_Trans(Operator):
    ...
    def trans_poll(self, bakemaster):
        ...
        if not bakemaster.drag_data_from == bakemaster.get_wh_childs_name(
    ...
    def get_destination_data(self, bakemaster):
        drag_data_to_getter = getattr(
            bakemaster, "get_active_%s" % bakemaster.drag_data_to)
    ...
    def get_source(self, bakemaster):
        drag_data_from_getter = getattr(
            bakemaster, "get_active_%s" % bakemaster.drag_data_from)
        data_from, containers_from, attr_from = drag_data_from_getter(
            bakemaster)

        bakemaster.wh_disable_drag(data_from, containers_from, attr_from)
    ...
    def execute(self, context):
        ...
        for index in selected_indexes:
            ...
            new_container = bakemaster.wh_copy(container_from,
        ...
        bakemaster.wh_disable_drag(
                destination_data, destination_containers,
                bakemaster.drag_data_from)
        bakemaster.wh_recalc_indexes(destination_data, "bakejobs")
...
class BM_OT_Global_WalkData_Move(Operator):
    """
    ...
    def after_execute(self, bakemaster, data, _, attr):
        """
        ...
        bakemaster.indexes_recalc(data, attr)
    ...
    def execute(self, context):
        ...
        if data is None or any([self.index == -1, self.new_index == -1]):
            ...
            # bakemaster.log("o0x0000", self.data_name, self)
            return {'CANCELLED'}
        ...
        active_container = getattr(bakemaster, "get_%s" % attr[:-1])(
            data, self.new_index)
